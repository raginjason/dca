...
test_devcontainer_up_called() {
    echo "Test: devcontainer up is called before exec zsh"
    TEMP="$(mktemp -d)"
    ...
    PATH="$TEMP/bin:$PATH" FAKE_DEVCONTAINER_LOG="$LOG" \
        XDG_CONFIG_HOME="$TEMP/xdg" sh "$DCA_ZSH" "$TEMP/workspace" 2>/dev/null
    exit_code=$?
    set -e
    ...
    assert_file_contains "$LOG" "zsh" "zsh was requested via devcontainer exec"
...
test_passthrough_args() {
    ...
    set +e
    PATH="$TEMP/bin:$PATH" FAKE_DEVCONTAINER_LOG="$LOG" \
        XDG_CONFIG_HOME="$TEMP/xdg" sh "$DCA_ZSH" "$TEMP/workspace" \
...
test_missing_devcontainer() {
    ...
    set +e
    output="$(PATH="$TEMP/fakebin" "$_sh" "$DCA_ZSH" "$TEMP/workspace" 2>&1)"
    exit_code=$?
    set -e
